        ...
        for j in range(len(galua_coef)):
            if nums[j] != numIfNull:
                ...
                if nums[j] + coefs[j] <= 255:
                    ...
                    galua.append(galua_fields[(nums[j] + coefs[j]) % 256])
...
nonlinear_coef = [252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233,
...
def getKeys(key):
    ...
    kk1=key[:int(len(key) / 2)]
    kk2=key[int(len(key) / 2) :]
    C = [] # константы
    F = [] # ячейки Фейстеля
    K = [kk1, kk2]
    ...
    # print(0, F[0], '\n')
    for i in range(32):
         F.append([ F[i][1], L(S(X( F[i][0], C[i])))])
        #  F.append(K)
        #  print(i+1, F[-1], '\n')
        #F.append(X(L(S(X(K[i], C[i]))), K[i+1]))
